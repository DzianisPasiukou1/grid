var Initializer = (function () {
	...
	Initializer.prototype.initGridOpt = function () {
		...
		if (this.scope.gridOptions.plugins === undefined) {
			...
			this.scope.gridOptions.plugins.push(new ExtGridActionsPlugin(this.scope.pluginActionOpt, this.$compile));
		}
		else {
			var isFindAct = false;

			for (var i = 0; i < this.scope.gridOptions.plugins.length; i++) {
				if (this.scope.gridOptions.plugins[i].constructor.name == 'ExtGridActionsPlugin') {
					isFindAct = true;
					break;
				}
			}
